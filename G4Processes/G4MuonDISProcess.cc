...
G4VParticleChange *G4MuonDISProcess::PostStepDoIt(const G4Track &track, const G4Step &step)
{
   ...
   if (track.GetParentID() == 0 && was_simulated == 0 && (fZEnd - pos.z()) > 1e-3) {
      if ((fZStart - pos.z()) > 1e-3) {
         ...
         LOG(INFO) << "Proposing DIS in volume path: " << geoVolumePath;
         ...
         if (pos.x() >= fXStart && pos.x() <= fXEnd && pos.y() >= fYStart && pos.y() <= fYEnd &&\
            ...
            LOG(INFO) << "DIS vertex in the x-y-z range, proceeding to DIS generation at "<< pos/cm
         ...
         // outside the desired x-y range!
            ...
            LOG(INFO) << "DIS vertex out of x-y-z range and/or volume, aborting the event at " << pos/cm;
                                                                ...
                                                                G4ForceCondition *condition)
{
   ...
   if (track.GetParentID() != 0 || (track.GetParentID() == 0 && pos.z() > fZEnd)) {
      *condition = NotForced;
      currentInteractionLength = DBL_MAX; // make sure this process won't be picked
      // reset parameters once primary particle passes z_end and we're done simulating DIS for the event
   ...
   } else {                                 // primary muon that didnt undergo DIS yet
      *condition = NotForced;
...
void G4MuonDISProcess::SimulateDIS(const G4Track &track)
{
   was_simulated = 1;
   G4ThreeVector pos = track.GetPosition();
   G4double Etrue = track.GetKineticEnergy();
   if (Etrue < 5. * GeV){
      return; // no DIS at this E - move upstream
   }
   ...
   myPythia->SetPARP(2, 2); // set minimum parton-parton transverse momentum to 2 M/GeV: why?
   ...
   myPythia->SetMRPY(1, R); // set some hard scattering parameter?

   std::map<int, const char *> mutype = {{-13, "gamma/mu+"}, {13, "gamma/mu-"}};

   myPythia->SetMSTU(11, 11); // Reduce output: maximum number of iterations for parton showers is set to 11.

   myPythia->Initialize("FIXT", mutype[pid], fNucleon, Etrue);

   myPythia->GenerateEvent();
   myPythia->Pyedit(2);
   // Integrated cross section for subprocess ISUB, based on the statistics accumulated so far
   fCrossSection = myPythia->GetPyint5()->XSEC[2][0];
   G4cout << "Total xsec " << fCrossSection << "\n";
   ...
   for (int itrk = 1; itrk <= myPythia->GetN(); ++itrk) {
      ...
      double mass = myPythia->GetP(itrk, 5) * GeV; // mass in GeV/c2 originally so convert to Pythia6's unit of MeV!
      double p = sqrt(dpx * dpx + dpy * dpy + dpz * dpz);
      G4double kineticEnergy = sqrt(p * p + mass * mass) - mass;
      ...
      // newTrack->SetTrackID(-1); // automatic assignment by G4
      newTrack->SetTouchableHandle(track.GetTouchableHandle()); // Important for geometry context
      // weight = in_muon_weight * density along trajectory *  cross-section
