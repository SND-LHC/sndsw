...
bool		gOptGenWeighted = false;       // weight gate
...
int main(int argc, char ** argv)
{
  ...
  // Keep the flux driver UNWEIGHTED while the driver is being configured.
  // GMCJDriver::Configure() (below) triggers the geometry max-path-length scan
  // (ROOTGeomAnalyzer::MaxPathLengthsFluxMethod, via SetScannerFlux) which pulls
  // "rays" through the geometry with fFlux->GenerateNext(). In weighted mode
  // GenerateNext() returns every raw flux entry, many of which miss the
  // geometry -> GenerateNext() returns false -> a flood of "Couldn't generate a
  // flux neutrino" warnings and an extremely slow scan. That scan is purely
  // geometric, so weighting buys nothing there. The user's --discard-weights choice is
  // (re)applied AFTER Configure(), just before the event-generation loop.
  // GenerateWeighted() is part of the GFluxI interface, so this works for
  // GSimpleNtpFlux / GNuMIFlux / GDk2NuFlux without any down-cast.
  flux_driver->GenerateWeighted(false);
  ...
  // Driver is now fully configured (geometry max-path-length scan and
  // probability-scale pre-computation are done). Apply the user's --discard-weights choice for
  // the actual event-generation loop. In weighted mode GSimpleNtpFlux does not use the weights
  // in the sampling procedure going straight to the ~ \sigma(E) part.
  flux_driver->GenerateWeighted(gOptGenWeighted);
  if ( gOptGenWeighted ) {
    LOG("gevgen_fnal", pNOTICE)
      << "Flux driver switched to WEIGHTED mode for event generation";
  }
...
void GetCommandLineArgs(int argc, char ** argv)
{
  ...
  // weighted flux generation?
  // With --discard-weights the flux driver's fGenWeighted is set true: GSimpleNtpFlux::GenerateNext()
  // returns every ray in order WITHOUT the wgt-based accept/reject, so negative /
  // signed / stat weights are carried through (via GFluxI::Weight() and the aux
  // passthrough, e.g. FLUKA_weight) instead of being used as sampling probabilities.
  gOptGenWeighted = parser.OptionExists("discard-weights");
  if(gOptGenWeighted) {
    LOG("gevgen_fnal", pNOTICE)
      << "Flux driver will run in WEIGHTED mode (fGenWeighted = true)";
  }
...
void PrintSyntax(void)
{
  LOG("gevgen_fnal", pFATAL)
   ...
   << "\n            [-z zmin_start]"
   << "\n            [--discard-weights] (disable flux wgt accept/reject; carry signed/stat weights)"
   << "\n            [--seed random_number_seed]"
   << "\n             --cross-sections xml_file"
