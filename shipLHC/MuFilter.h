...
class MuFilter : public FairDetector
{
    ...
    /** Getposition **/
                 ...
                 Float_t GetCorrectedTime(Int_t id, Int_t c, Double_t t, Double_t L, Float_t QDC=-1.);
		...
		std::vector<float> GetConfParVector(TString name){return conf_vectors[name];}
	...
	private:
		...
		std::map<TString,std::vector<float>> conf_vectors;
