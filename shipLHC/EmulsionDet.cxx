...
void EmulsionDet::ConstructGeometry()
{
	...
	int NTungstenPlates = conf_ints["EmulsionDet/n_tungsten_plates"];
	...
	//    //
	  ...
	  The plastic layers in Wall1, Brick 1 are the same thicknes as the plastic base in TI18 conf.
	...
	*/
	...
	float UpstreamPlasticThickness = (NPlates-NTungstenPlates)*AllPlateWidth;
	// it is a little thinner for Wall2, where the pлastic plates have 2 thicknesses
	float correction_W2 = NTungstenPlates/2*PlasticBaseThickness/3.;
	...
	if(fNWall==5) NTungstenPlates=0;// to build the TI18 geometry

	for(Int_t n=0; n<NPlates; n++)
	{
		if (n==0 && NTungstenPlates!=0){// testbeam 2024
		  volBrick->AddNode(volPassive_plastic, n, new TGeoTranslation(0,0,-BrickZ/2 + UpstreamPlasticThickness/2));// upstream plastic
		...
		if (n>=(NPlates-NTungstenPlates) || NTungstenPlates==0) { // instrumented part
		  volBrick->AddNode(volPassive, n, new TGeoTranslation(0,0,-BrickZ/2 + EmPlateWidth + PassiveThickness/2 + n*AllPlateWidth));// 1x
		  if (NTungstenPlates!=0){
		    volBrick_W2->AddNode(volPassive, n, new TGeoTranslation(0,0,-BrickZ/2 + (1+((n+1)%2)/3.)*EmPlateWidth + PassiveThickness/2 + accumulatve_width)); //interchange 1x or 4/3x
	...
	if (NTungstenPlates!=0) volBrick_W2->SetVisibility(kTRUE);// testbeam 2024
...
//alignment
	...
	for(int k= 0; k< fNRow; k++)
	  ...
	  if (NTungstenPlates!=0){ // testbeam 2024 
