...
import ROOT,os,subprocess,sys,operator
import atexit
import time
import ctypes
import pickle
...
### Additions for SciFi Alignment with Millepede-II, Ceren 

# XXX functions are based on SHiP DT Alignment with Millepede by Stefan Bieschke. 
# They are adjusted for SND@LHC SciFi alignment.
...
def TracksforGBL(Nev=options.nEvents,NbinsRes=100,xmin=-2000.,alignPar=False,unbiased = True,minEnergy=False,remakeClusters=options.remakeScifiClusters,nproc=1):
    projs = {1:'V',0:'H'}
    for s in range(1,6):
     for o in range(2):
        for p in projs:
...
def GBL_refit(nEvent=-1,nTot=1000,PR=13,minP=10.,pede_results = None, cpp_pede = None, reshape = False):
    ...
    This function is based on the Scifi_residuals(...) method in this script.
...
def tube_id_module_pede_labels(id):  ##FIX THIS
