...
class MuFilterHit : public SndlhcHit
{
 ...
 /** Destructor **/
    ...
    /** Output to screen **/
    void Print() const;
    Float_t GetEnergy(Bool_t use_short=kFALSE);
    Float_t SumOfSignals(char* opt,Bool_t mask=kTRUE);
    std::map<TString,Float_t> SumOfSignals(Bool_t mask=kTRUE);
    std::map<Int_t,Float_t> GetAllSignals(Bool_t mask=kTRUE,Bool_t positive=kTRUE,Bool_t use_short=kFALSE);
    std::map<Int_t,Float_t> GetAllTimes(Bool_t mask=kTRUE,Bool_t positive=kTRUE,Bool_t use_short=kFALSE);
    Float_t  GetDeltaT(Bool_t mask=kTRUE,Bool_t positive=kTRUE,Bool_t use_short=kFALSE);
    Float_t  GetFastDeltaT(Bool_t mask=kTRUE,Bool_t positive=kTRUE,Bool_t use_short=kFALSE);
    Float_t  GetImpactT(Bool_t mask=kTRUE,Bool_t positive=kTRUE,Bool_t use_short=kFALSE);
    Float_t  GetImpactXpos(Bool_t mask=kTRUE,Bool_t positive=kTRUE,Bool_t use_short=kFALSE,Bool_t isMC=kFALSE);
